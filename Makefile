...
help:
	...
	@echo "  make k8s-lint [SERVICE=main-service] [K8S_ENVS='dev test prod']"
	@echo "  make k8s-template [SERVICE=main-service] [ENV=dev|test|prod] [IMAGE_TAG=<tag>]"
	@echo "  make k8s-context [K8S_KUBECONFIG=~/.kube/ai-trust-game-pi.yaml]"
	@echo "  make k8s-build-push SERVICE=main-service ENV=dev IMAGE_TAG=<tag>"
	@echo "  make k8s-apply [SERVICE=main-service] [ENV=dev|test|prod] [IMAGE_TAG=<tag>]"
	@echo "  make k8s-deploy [SERVICE=main-service] [ENV=dev|test|prod]"
	@echo "  make k8s-delete [SERVICE=main-service] [ENV=dev|test|prod]"
	@echo "  make k8s-apply-entry [ENV=dev|test|prod]"
	@echo "  make k8s-delete-entry [ENV=dev|test|prod]"
	@echo "  make k8s-status"
	@echo "  make manual-deploy [SERVICE=main-service] [ENV=dev|test|prod]"
