...
help:
	...
	@echo "  make k8s-lint [SERVICE=game-service] [K8S_ENVS='dev test prod']"
	@echo "  make k8s-template [SERVICE=game-service] [ENV=dev|test|prod] [IMAGE_TAG=<tag>]"
	@echo "  make k8s-context [K8S_KUBECONFIG=~/.kube/ai-trust-game-pi.yaml]"
	@echo "  make k8s-build-push SERVICE=game-service ENV=dev IMAGE_TAG=<tag>"
	@echo "  make k8s-apply [SERVICE=game-service] [ENV=dev|test|prod] [IMAGE_TAG=<tag>]"
	@echo "  make k8s-deploy [SERVICE=game-service] [ENV=dev|test|prod]"
	@echo "  make k8s-delete [SERVICE=game-service] [ENV=dev|test|prod]"
	@echo "  make k8s-apply-entry [ENV=dev|test|prod]"
	@echo "  make k8s-delete-entry [ENV=dev|test|prod]"
	@echo "  make k8s-status"
	@echo "  make manual-deploy [SERVICE=game-service] [ENV=dev|test|prod]"
