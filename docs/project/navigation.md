# Project Navigation

This document works as a table of contents.

Use it to find the high-level documentation entry points.
Detailed code-near documentation is linked from the owning app, service, shared-code, and infrastructure READMEs.

## Project

- [Project Description and Goals](description.md)
- [Project Plan](plan.md)

## Development

- [Agent working contract and documentation rules](../development/AGENT.md)
- [Agent planning playbook](../development/AGENT-planning.md)
- [Current agent phase plan](../development/AGENT-plan.md)
- [AGENT phase planning notes](../development/AGENT-notes.md)

## Commands

- [Project commands](commands.md)

## Architecture

- [Architecture Guidelines](../architecture/guidelines.md)
- [Backend Architecture](../architecture/backend.md)
- [Frontend Architecture](../architecture/frontend.md)

## Apps

- [Trust Game App](../../apps/trust-game-app/README.md)

## Backend

- [Gateway Service](../../services/gateway-service/README.md)
- [Auth Service](../../services/auth-service/README.md)
- [Game Service](../../services/game-service/README.md)
- [Logging Service](../../services/logging-service/README.md)
- [Audit Service](../../services/audit-service/README.md)
- [Shared Service Code](../../services/shared/README.md)

## Infrastructure

- [Infrastructure](../../infrastructure/README.md)
- [Kubernetes Deployment Overview](../deployment/k8s.md)
- [GitHub Actions Workflows](../../.github/workflows/README.md)

## Diagrams

- [Frontend Flow Diagrams](../diagrams/frontend-uml.md)
